    ...
    <div class="mt-3 border border-[#E5E3DB] bg-white">
        <div class="flex items-start justify-between border-b border-[#E5E3DB] bg-[#FCFBF8] px-8 py-6">
            <div>
                ...
                <h1 class="font-display text-3xl font-semibold text-[#16213A]">{{ $teacher['name'] ?? 'Budi Santoso' }}</h1>
                <p class="mt-1 font-mono text-xs text-slate-500">NIP {{ $teacher['nip'] ?? '198501012024' }}</p>
            </div>

            <a href="{{ route('teachers.edit', $teacher['id'] ?? 1) }}"
        ...
        <div class="flex justify-end gap-4 border-t border-[#E5E3DB] px-8 py-5">
            ...
            <form action="{{ route('teachers.destroy', $teacher['id'] ?? 1) }}" method="POST"
